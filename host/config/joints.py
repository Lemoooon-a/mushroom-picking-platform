...
- 肩关节输出轴绝对角 100 度为逻辑 0 度，逻辑范围 -60 到 +70 度；
- 肘关节输出轴绝对角 158 度为逻辑 0 度，方向反转，逻辑范围 -152 到 +152 度；
...
SHOULDER_JOINT_CONFIG = JointConfig(
    ...
    min_position_rad=math.radians(-60.0),  # OA=40 度，对应最小逻辑角 -60 度。
    max_position_rad=math.radians(70.0),  # OA=170 度，对应最大逻辑角 +70 度。
...
ELBOW_JOINT_CONFIG = JointConfig(
    ...
    min_position_rad=math.radians(-152.0),  # OA=310 度，对应最小逻辑角 -152 度。
    max_position_rad=math.radians(152.0),  # OA=6 度，对应最大逻辑角 +152 度。
    max_velocity_rad_s=math.radians(50.0),  # 最大逻辑关节速度为每秒 50 度。
    position_tolerance_rad=math.radians(0.1),  # 误差不超过 0.1 度时不重复发位置命令。
    moving_velocity_threshold_rad_s=math.radians(0.05),  # 超过每秒 0.05 度视为仍在运动。
)
