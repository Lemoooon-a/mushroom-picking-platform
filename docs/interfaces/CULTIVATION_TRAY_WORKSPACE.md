...
arm-local 单一工作区与 200 mm 进入工作区安全高度都不是培养槽边界来源；安全高度与当前 Tray
Z 上限数值相同，但配置来源和用途独立。
...
| Arm-local workspace validity | arm-local frame | 当前 Slide 下局部解是否位于唯一合法矩形 | `host/config/project/workspace_planning.py` |
| Robot motion envelope policy | Base Z + 轴逻辑位置 | startup/return 与区外进入中间阶段采用什么固定策略 | `host/config/project/robot_motion_envelope.py` |
...
集中 startup/return pose 和 workspace-entry clearance。当前仅是软件阶段策略，不是完整碰撞模型或
经过认证的机械安全包络。
...
数值来源：本机用户确认的 Base 工作区；X `[100, 600] mm`、Y `[150, 800] mm`。Z 是最终 TCP 在 Base frame 中的绝对
...
## 5. Startup Safe Pose and Workspace-Entry Clearance
...
### Workspace-Entry Clearance

- `DIRECT` 与 `LOWER` 的最终落点等于已通过培养槽检查的请求目标；
- `LIFT` 与 `TRANSIT` 可以高于培养槽正常 Z 上限，不逐阶段调用 `TrayWorkspace`；
- clearance 为 `RobotMotionEnvelopeConfig.workspace_entry.clearance_base_z_mm=200 mm` 的绝对 Base TCP
  Z 最低高度，不是相对增量、Z Home、从 Tray `z_max` 推导的值或 target `+200 mm`；
- 所有中间阶段仍受轴软限位、运动包络策略、工作区进入规则及完整 FK 验证约束。
...
省略 `--execute` 仍是只读预览。`workspace` 命令显示 Tray、arm-local workspace、Robot motion
